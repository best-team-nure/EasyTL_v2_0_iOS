...
class ViewController: UIViewController {
    ...
    @IBAction func button1(_ sender: Any) {
        ...
        questionText.text = "Нажать кнопку старт, прочитать теорию и начать проходить тест по пункту теории. Также вы можете пройти тест из 20 вопросов нажав на кнопку \"Быстрый тест\""
    }
     ...
     @IBAction func a1Quick(_ sender: Any) {
         ...
         switch currentQuick {
            ...
            case 1:
                ...
                createAlert(title: "Ошибка", message: "Вот это поворот! Вообще-то в пункте 10.10 сказано: Запрещается движение транспортных средств задним ходом на участках дорог с ограниченным обзорностью или недостаточной видимостью")
                break
            case 2:
            ...
            case 5:
             ...
             case 7:
                 ...
                 createAlert(title: "Ошибка", message: "Так-так-так, кажется кто-то не читал пункт 14.2, в котором сказано: Перед началом обгона водитель должен убедиться в том, что водитель транспортного средства, которое двигается впереди по той самой полосе, не подал сигнал о намерении поворота (перестраивание) налево.")
                 break
             case 8:
                 gifQuickConstr(gif: "q8-a1",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Ну и куда ты собрался??? Пункт 14.6 предупреждает тебя, что: Обгон колонны транспортных средств, позади которой движется транспортное средство с включенным проблесковым маяком (кроме оранжевого), запрещен!")
                 break
             case 9:
                 gifQuickConstr(gif: "q9-a1",repeatcount: 1)
                  createAlert(title: "Ошибка", message: "Ну куда ты собрался обгонять? Сказано же в пункте 14.6, что: Обгон запрещен на дорогах, которые имеют две и более полосы для движения в одном направлении")
                 break
             case 10:
                 gifQuickConstr(gif: "q10-a1",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Ну нельзя же здесь этого делать!! В пункте 14.6 говориться, что: Обгон запрещен на перекрестках.")
                 break
             case 11:
                 gifQuickConstr(gif: "q11-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "И что ждать? Вот что ждать? Почитай пункт 14.2: Перед началом обгона водитель должен убедиться в том, что после обгона сможет, не создавая препятствий обгоняемому им транспортному средству, вернуться на занимаемую полосу. Убедился - езжай!")
                 break
             case 12:
                 gifQuickConstr(gif: "q12-a0",repeatcount: 1)
                  createAlert(title: "Ошибка", message: "Неееет, только не останавливайтся здесь! Говорится же в пункте 15.9, что: Остановка запрещается в местах, где расстояние между сплошной линией разметки, разделительной полосой или противоположным краем проезжей части и транспортным средством, которое остановилось, менее 3 м.")
                 break
             case 13:
             ...
             case 14:
                 ...
                 createAlert(title: "Ошибка", message: "Скажи “нет-нет-нет” остановкам на мосту! Ведь даже пункт 15.9 говорит, что: Остановка запрещается на эстакадах, мостах, путепроводах и под ними, а также в туннелях.")
                 break
             case 15:
                 gifQuickConstr(gif: "q15-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Тоже мне вопрос! Понимаю тебя, сложно ответить. Но ты все таки должен знать что в пункте 15.9 сказанно: Остановка запрещается в местах, где расстояние между сплошной линией разметки, разделительной полосой или противоположным краем проезжей части и транспортным средством, которое остановилось, менее 3 м.")
                 break
             case 16:
                 gifQuickConstr(gif: "q16-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Даже если оооооочень просят, ну не стоит нарушать ПДД! Пункт 15.9 подскажет тебе, что: Остановка запрещается                    на эстакадах, мостах, путепроводах и под ними, а также в туннелях.")
                 break
             case 17:
                 gifQuickConstr(gif: "q17-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Ну ты чего? Теперь поворачивать налево отказываешься? я конечно все понимаю, но пункт 16.10 говорит, что: На перекрестке, где движение регулируется свето­фором с дополнительной секцией, водитель, находя­щийся на полосе, с которой производится поворот, должен продолжать движение в направлении, ука­занном стрелкой, включенной в дополнительной сек­ции.")
                 break
             case 18:
                 gifQuickConstr(gif: "q18-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Притормози! Куда так спешишь? Ведь даже в пункте 16.12, что: Преимущество в движении на нерегулируемых перекрестках, на которых организовано круговое движение и которые обозначены соответствующим дорожным знаком, предоставляется транспортным средствам, которые уже движутся по кругу")
                 break
             case 19:
     ...
     @IBAction func a2Quick(_ sender: Any) {
         ...
         switch currentQuick {
             case 0:
                 ...
                 createAlert(title: "Ошибка", message: "Он что, пуп земли? В пункте 10.8 сказано: Если в месте въезда на дорогу есть полоса разгона, водитель должен двигаться по ней и вливаться в транспортный поток, уступая дорогу транспортным средствам, движущимся по этой дороге!")
                 break
             ...
             case 2:
                 ...
                 createAlert(title: "Ошибка", message: "Не стоит водителю путать право и лево! Обязательно почитай пункт 10.11, в котором кстати говориться: В случае если траектории движения транспортных средств пересекаются, а очередность проезда не обусловлена настоящими Правилами, уступить дорогу должен водитель, к которому транспортное средство приближаются с правой стороны.")
                 break
             ...
             case 4:
                 ...
                 createAlert(title: "Ошибка", message: "Ну куда ты спешишь, дружище?Хочешь штраф заработать или что? Лучше обрати внимание на пункт 10.2, где сказано: Выезжая на дорогу из жилой зоны, дворов, мест стоянки, автозаправочных станций и других примыкающих территорий, водитель должен перед проезжей частью или тротуаром уступить дорогу пешеходам и транспортным средствам, движущимся по ней.")
                 break
                 
             case 5:
                 gifQuickConstr(gif: "q6-a0",repeatcount: 1)
                 createAlert(title: "Ошибка", message: "Так и аварию создать можно!! Прочитай внимательно пункт 10.3. Там написано: При перестроении водитель должен уступить дорогу транспортным средствам, движущимся в попутном направлении по той полосе, на которую он намерен перестроиться.")
                 break
             case 7:
             ...
             case 13:
                  gifQuickConstr(gif: "q13-a0",repeatcount: 1)
                   createAlert(title: "Ошибка", message: "Ну кто останавливается так близко к пешеходному перекрестку? Ты что забыл, что в пункте 15.9 сказано: Остановка запрещается на пешеходных переходах и ближе 10 м от них с обеих сторон, кроме случаев предоставления преимущества в движении")
                 break
             case 14:
             ...
             case 20:
                 gifQuickConstr(gif: "q20-a0",repeatcount: 1)
                  createAlert(title: "Ошибка", message: "Ой, трамвай же такой огромный и опасный! Следуй пожалуйста пункту 16.6, в котором сказано: Поворачивая налево или разворачиваясь при зеле­ном сигнале основного светофора, водитель нерель­сового транспортного средства обязан уступить до­рогу трамваю попутного направления, а также транс­портным средствам, движущимся во встречном на­правлении прямо или поворачивающим направо.")
             break
             default:
                 break
         }
